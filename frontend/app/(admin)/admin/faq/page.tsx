...
const emptyForm = { question: '', answer: '', questionBn: '', answerBn: '' };
...
export default function AdminFaqPage() {
  ...
  async function save(e: React.FormEvent) {
    ...
    const questionBn = form.questionBn.trim();
    const answerBn = form.answerBn.trim();
      ...
      <form onSubmit={save} className="mt-8 space-y-4 border border-ocean/10 bg-white p-6">
        ...
        <label className="block text-sm font-medium text-ocean">
          ...
          <input
            ...
            className="field mt-1"
            placeholder="ভগ্নাংশ মালিকানা কীভাবে কাজ করে?"
            maxLength={300}
          />
        </label>
        <div>
          <div className="text-sm font-medium text-ocean">Answer (EN)</div>
          <RichTextEditor
            key={(editingId || 'new-faq') + '-en'}
            value={form.answer}
        ...
        <div>
          <div className="text-sm font-medium text-ocean">Answer (BN)</div>
          <RichTextEditor
            key={(editingId || 'new-faq') + '-bn'}
            value={form.answerBn}
            onChange={(answerBn) => setForm({ ...form, answerBn })}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {item.questionBn ? (
                    <div className="mt-1 text-sm text-ocean/70">{item.questionBn}</div>
                  ) : null}
                </div>
