...
const RichTextEditor = dynamic(() => import('@/components/RichTextEditor'), {
  ssr: false,
  loading: () => <div className="field mt-1 min-h-[180px] animate-pulse bg-pearl/60" />
});
...
const emptyForm = { title: '', body: '', titleBn: '', bodyBn: '' };
      ...
      <form onSubmit={save} className="mt-8 space-y-4 border border-ocean/10 bg-white p-6">
        ...
        <label className="block text-sm font-medium text-ocean">
          ...
          <input
            ...
            className="field mt-1"
            placeholder="৯. প্রযোজ্য আইন"
            maxLength={200}
          />
        </label>
        <div>
          <div className="text-sm font-medium text-ocean">Body (EN)</div>
          <RichTextEditor
            key={(editingId || 'new-terms') + '-en'}
            value={form.body}
        ...
        <div>
          <div className="text-sm font-medium text-ocean">Body (BN)</div>
          <RichTextEditor
            key={(editingId || 'new-terms') + '-bn'}
            value={form.bodyBn}
            onChange={(bodyBn) => setForm({ ...form, bodyBn })}
            placeholder="বাংলায় অনুচ্ছেদ লিখুন।"
            minHeightClass="min-h-[180px]"
          />
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {item.titleBn ? (
                    <p className="mt-1 text-sm text-ocean/70">{item.titleBn}</p>
                  ) : null}
                </div>
