...
export default function AdminSuitePlansPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  async function load() {
    ...
    setError("");
    try {
      const res = await fetch(`${API_URL}/suites/${suiteId}/plans`);
  ...
  async function createPlan(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`${API_URL}/timeshares`, {
        method: "POST",
        headers: {
  ...
  async function deletePlan(id: string) {
    setError("");
    try {
      const res = await fetch(`${API_URL}/timeshares/${id}`, {
        method: "DELETE",
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setCreateForm({
                  ...createForm,
                  timeFraction: Number(e.target.value),
                })
              }
              className="mt-1 w-full rounded border border-ocean/20 px-2 py-1"
              placeholder="0.1"
          ...
          <div>
            ...
            <div className="mt-1 rounded border border-ocean/20 bg-ocean/5 px-2 py-1 text-ocean/70">
              {(
                (createForm.timeFraction ??
                  (createForm.daysPerMonth
                    ? createForm.daysPerMonth / 30
                    : 0)) * 100
              ).toFixed(1)}
              %
            </div>
          ...
          <tbody>
            {items.map((p) => (
              <tr key={p.id} className="border-t border-ocean/10">
                ...
                <td className="p-3">
                  {((p.timeFraction ?? 0) * 100).toFixed(1)}%
                </td>
                  ...
                  {editingId === p.id ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="rounded bg-ocean px-3 py-1 text-white"
                      >
                        Save
                      </button>
                      <button
                        onClick={() => setEditingId(null)}
                        className="rounded border border-ocean px-3 py-1 text-ocean"
                      >
                        Cancel
                      </button>
                    </div>
