...
export default function AdminPolicyPage() {
  const [policy, setPolicy] = useState<RevenuePolicy>({
    taxRate: 0.1,
  ...
  async function load() {
    ...
    setOk("");
    try {
      const res = await fetch(`${API_URL}/settings/revenue-policy`);
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16">
      ...
      <p className="mt-2 text-ocean/80">
        Configure global rates used to compute distributable revenue.
      </p>
      {error && (
      ...
      <form
        onSubmit={save}
        className="mt-6 space-y-4 rounded border border-gold/30 bg-white p-6"
      >
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setPolicy({ ...policy, taxRate: Number(e.target.value) })
            }
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setPolicy({
                ...policy,
                maintenanceReserveRate: Number(e.target.value),
              })
            }
            className="mt-1 w-full rounded border border-ocean/20 px-2 py-1"
          />
        ...
        <div className="pt-2">
          ...
          <button
            type="button"
            onClick={load}
            className="ml-2 rounded border border-ocean px-4 py-2 text-ocean"
          >
